...
class McApi(DownloadApi):
    ...
    def _get_download_client_task(self, versions_files: MinecrafFiles, version_name: str):
        client = versions_files["downloads"]["client"]
        file_path = f".minecraft/versions/{version_name}"
    ...
    async def _put_download_libraries_task(self, librariesList: List[LibrariesItem], queue: asyncio.Queue) -> None:
        ...
        for libraries in librariesList:
            ...
            if "classifiers" in libraries["downloads"]:
                await queue.put(self.__download_libraries(libraries["downloads"]["classifiers"][f"natives-{os_name}"], progress_task))
                continue
    ...
    def _extract_natives(self, versions_files: MinecrafFiles, version_name: str):
        ...
        for lib in versions_files["libraries"]:
            ...
            lwjgl_lib.append(f".minecraft/assets/libraries/{lib["downloads"]['artifact']["path"]}")
        ...
        for file_path in flie_list:
            thread = Thread(target=self._extract_natives_task, args=(file_path, f".minecraft/versions/{version_name}/{version_name}_natives"))
            thread.daemon = True
    ...
    async def download(self, version: str, version_name: str | None = None) -> None:
        ...
        if not os.path.exists(f".minecraft/versions/{version_name}"):
            os.makedirs(f".minecraft/versions/{version_name}")
        
        versions_files_json_path = f".minecraft/versions/{version_name}/{version_name}.json"
        versions_files = None
        ...
        await task_queue.put(self._get_download_client_task(versions_files, version_name))
        ...
        self._extract_natives(versions_files, version_name)
        self.progress.stop()
        self.progress = None
...
class FabricApi(DownloadApi):
    ...
    async def _put_download_libraries_task(self, loader_version: FabricLoaderVersions, queue: asyncio.Queue):
        ...
        for lib in libs:
            ...
            if "sha1" not in lib:
                await queue.put(self.__download_libraries(lib_url, path_dict["path"], path_dict["lib_name"], None, lib["size"], progress_task))
                continue
            
            await queue.put(self.__download_libraries(lib_url, path_dict["path"], path_dict["lib_name"], lib["sha1"], lib["size"], progress_task))

    async def download_from_file(self, fabric_loader_files: FabricLoaderFiles, version_name: str) -> None:
        loader_versions_id = fabric_loader_files["id"]
        self.log.info(f"获取版本 {loader_versions_id} 完成")

        fabric_loader_files_json_path = f".minecraft/versions/{version_name}/{fabric_loader_files["inheritsFrom"]}_fabric.json"
    ...
    async def download(self, loader_version: str, mc_version: str, version_name: str | None = None) -> None:
        ...
        await self.download_from_file(await self.get_files(await self.get_version(loader_version, mc_version)), version_name)